...
This card applies to `Dionysus/evals/` and every file below it.

## Role

This skeleton port captures seed-garden eval pressure before it is accepted,
rejected, or normalized by `aoa-evals`.

`aoa-evals` owns central verdict, scoring, regression, and proof doctrine
authority. This port owns only seed-local intake, cases, fixtures, suites,
...
- Keep seed sources, wave manifests, closure notes, planting protocol, and
  lineage trace in `Dionysus`.
