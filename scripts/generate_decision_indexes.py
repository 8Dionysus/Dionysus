#!/usr/bin/env python3
"""Generate docs/decisions lookup indexes from Dionysus decision metadata."""
...
DECISION_PREFIX = "DION-SEED-D"
DECISION_RE = re.compile(r"^DION-SEED-D-(\d{4})-[a-z0-9][a-z0-9-]*\.md$")
DECISION_ID_RE = re.compile(r"^DION-SEED-D-\d{4}$")
...
INDEX_PATHS = (
    ...
    INDEX_DIR / "by-seed-surface.md",
    INDEX_DIR / "by-owner-lane.md",
...
def collect_decision_records(repo_root: Path = REPO_ROOT) -> tuple[list[DecisionRecord], list[tuple[str, str]]]:
    ...
    for path in sorted(decision_root.glob("*.md")):
        ...
        if not path.name.startswith(f"{DECISION_PREFIX}-"):
            issues.append((rel, "top-level decision markdown must use DION-SEED-D-####-slug.md"))

    for path in sorted(decision_root.glob(f"{DECISION_PREFIX}-*.md")):
        rel = path.relative_to(repo_root).as_posix()
        match = DECISION_RE.match(path.name)
        if not match:
            issues.append((rel, "filename must match DION-SEED-D-####-slug.md"))
            continue
        ...
        seed_surfaces = _metadata_value(metadata, "Seed surfaces", rel, issues)
        owner_lanes = _metadata_value(metadata, "Owner lanes", rel, issues)
        ...
        if decision_id and not DECISION_ID_RE.match(decision_id):
            issues.append((rel, "Decision ID must match DION-SEED-D-####"))
        if original_date:
            try:
        ...
        if decision_id and original_date and title:
            records.append(
                DecisionRecord(
                    ...
                    seed_surfaces=_split_values(seed_surfaces),
                    owner_lanes=_split_values(owner_lanes),
    ...
    if numbers:
        ...
        if numbers != expected_numbers:
            issues.append((DECISIONS_DIR.as_posix(), "decision numbers must be contiguous from DION-SEED-D-0001"))
...
def validate_index_contract(repo_root: Path = REPO_ROOT) -> list[tuple[str, str]]:
    ...
    text = path.read_text(encoding="utf-8")
    required = [
        "schema_version: dion_seed_decision_index_contract_v1",
        "authority: docs/decisions/AGENTS.md",
        "source_records: docs/decisions/DION-SEED-D-*.md",
...
def _table(records: list[DecisionRecord], *, columns: tuple[str, ...]) -> str:
    ...
    for record in records:
        ...
        for column in columns:
            ...
            elif column == "Seed surfaces":
                values.append(", ".join(record.seed_surfaces))
            elif column == "Owner lanes":
                values.append(", ".join(record.owner_lanes))
...
def render_index_files(records: list[DecisionRecord]) -> dict[Path, str]:
    ...
    return {
        INDEX_DIR / "README.md": (
            ...
            "- [By seed surface](by-seed-surface.md)\n"
            "- [By owner lane](by-owner-lane.md)\n"
        ...
        INDEX_DIR / "by-seed-surface.md": _grouped_index(
            "Decisions By Seed Surface",
            _group_by(records, "seed_surfaces"),
        ),
        INDEX_DIR / "by-owner-lane.md": _grouped_index(
            "Decisions By Owner Lane",
            _group_by(records, "owner_lanes"),
        ),
