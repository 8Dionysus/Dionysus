...
DEFAULT_SUBJECT = REPO_ROOT / "generated" / "seed_route_map.min.json"
...
def _assert_manifest_matches_subject(manifest: Path, subject: Path) -> None:
    ...
    if payload.get("artifact_class") != EXPECTED_ARTIFACT_CLASS:
        raise ValueError(f"manifest artifact_class must be {EXPECTED_ARTIFACT_CLASS}")
    if payload.get("owner_repo") != "Dionysus":
        raise ValueError("manifest owner_repo must be Dionysus")
...
def _assert_public_safe_subjects(manifest: Path, subject: Path) -> None:
    ...
    if leaks:
        raise ValueError("seed route bundle subjects contain private or machine-local markers: " + ", ".join(leaks))
...
def _assert_expected_controls(verify: dict[str, Any], identity: dict[str, Any]) -> None:
    ...
    c2pa_reason = str(deferred.get("c2pa", {}).get("reason") or "")
    if "seed-pack credential class" not in c2pa_reason:
...
def _registry_roundtrip(
    ...
    *,
    ...
    registered = artifact_bundles.write_bundle_registry_record(
        ...
        consumer_refs=["Dionysus:seed-route-readmodel"],
        evidence_refs=[evidence_ref],
    )
    latest = artifact_bundles.read_bundle_registry(registry_dir, artifact_class=EXPECTED_ARTIFACT_CLASS)
    latest_record = latest.get("latest_by_artifact_class", {}).get(EXPECTED_ARTIFACT_CLASS)
    return {
        "ok": bool(
            registered.get("ok")
...
def _verify_missing_abi(artifact_bundles: Any, abyss_repo_root: Path, bundle_dir: Path, tmp_root: Path) -> dict[str, Any]:
    ...
    return {
        "ok": verification.get("ok") is False and artifact_bundles.ABI_SIDECAR in verification.get("missing", []),
...
def _verify_private_route_marker(manifest: Path, subject: Path, tmp_root: Path) -> dict[str, Any]:
    candidate = tmp_root / "private-seed-route-map.min.json"
    shutil.copyfile(subject, candidate)
    candidate.write_text(candidate.read_text(encoding="utf-8") + "\nTOKEN=private-negative\n", encoding="utf-8")
    try:
        _assert_public_safe_subjects(manifest, candidate)
    except ValueError as exc:
        return {"ok": "seed route bundle subjects contain private or machine-local markers" in str(exc), "error": str(exc)}
    return {"ok": False, "error": "private seed route marker was not detected"}
...
def _verify_terminal_registry_state(artifact_bundles: Any, bundle_dir: Path, tmp_root: Path) -> dict[str, Any]:
    ...
    revoked = artifact_bundles.write_bundle_registry_record(
        ...
        revocation_reason="Dionysus seed route terminal-state rehearsal",
    )
    after_revoke = artifact_bundles.read_bundle_registry(registry_dir, artifact_class=EXPECTED_ARTIFACT_CLASS)
    return {
...
def _run_adversarial_checks(
    ...
    with tempfile.TemporaryDirectory(prefix="dionysus-seed-route-negative-", dir=_default_tmp_root()) as tmp:
        tmp_root = Path(tmp)
        checks = {
            ...
            "private_route_marker": _verify_private_route_marker(manifest, subject, tmp_root),
...
def _validate_in_bundle_dir(
    ...
    *,
    ...
    return {
        ...
        "schema": "dionysus_abyss_machine_seed_route_artifact_bundle_validation_v1",
...
def validate_bundle(
    ...
    *,
    ...
    if bundle_dir is not None:
        target_registry = registry_dir or bundle_dir.parent / "seed-route-readmodel-registry"
        return _validate_in_bundle_dir(manifest, subject, bundle_dir, target_registry, clean=clean)

    tmp_root = _default_tmp_root()
    with tempfile.TemporaryDirectory(prefix="dionysus-seed-route-bundle-", dir=tmp_root) as tmp:
        target = Path(tmp) / "seed-route-readmodel"
        target_registry = Path(tmp) / "seed-route-readmodel-registry"
        return _validate_in_bundle_dir(manifest, subject, target, target_registry, clean=False)


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate Dionysus seed route map through OS Abyss artifact bundles."
    )
    ...
    if args.json:
        ...
        print(
            "[ok] abyss-machine Dionysus seed route artifact bundle verified: "
