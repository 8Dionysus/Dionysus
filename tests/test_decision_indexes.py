...
def test_by_number_index_exposes_canonical_id_and_path() -> None:
    ...
    assert "`docs/decisions/DION-SEED-D-0001-seed-decision-rationale-lane.md`" in by_number
...
def test_grouped_indexes_are_lookup_bullets_not_repeated_ledgers() -> None:
    ...
    assert "- [DION-SEED-D-" in by_surface
    assert "(`docs/decisions/DION-SEED-D-" in by_surface


def test_decision_id_must_match_filename_prefix(tmp_path: Path) -> None:
    decision_dir = tmp_path / "docs" / "decisions"
    decision_dir.mkdir(parents=True)
    decision = decision_dir / "DION-SEED-D-0001-example.md"
    decision.write_text(
        "\n".join(
            [
                "# Example",
                ...
                "## Index Metadata",
                "",
                "- Decision ID: DION-SEED-D-0002",
                "- Original date: 2026-06-04",
                "- Surface classes: docs/decisions",
                "- Seed surfaces: decision lane",
                "- Owner lanes: Dionysus",
                "- Guard families: seed-garden boundary",
                "- Posture: accepted",
                "",
            ]
        ),
...
def test_original_date_must_use_canonical_yyyy_mm_dd(tmp_path: Path) -> None:
    ...
    assert ("docs/decisions/DION-SEED-D-0001-example.md", "Original date must use YYYY-MM-DD") in issues
