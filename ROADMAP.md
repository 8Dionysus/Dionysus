...
- Select an encrypted local vault design and retention policy.
- Implement one provider-neutral session recorder and transcript adapter.
- Pilot baseline, depth, and counterportrait interviews with explicit review.
- Measure claim correction, disagreement, evidence traceability, and participant
  burden.

Exit condition: a person can inspect and correct every retained claim without
reading raw system internals.

## Phase 3 — selective context

- Generate the three compact portrait projections from reviewed claims.
- Add purpose-specific disclosure and expiry controls.
- Integrate with other AoA organs only through explicit, narrow contracts.

Exit condition: no consumer requires the entire vault or silently becomes a
second authority over identity.
